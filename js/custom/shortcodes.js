/* Girlss HTML Theme Shortcodes JS*/
/*
...
Girlss_win.on('beforeunload', function() {
    "use strict";
    // Show preloader
...
function trx_addons_scroll_actions() {
    ...
    jQuery('[data-animation^="animated"]:not(.animated)').each(function() {
        "use strict";
        if (jQuery(this).offset().top < scroll_offset + Girlss_win.height())
...
jQuery(document).on('action.add_googlemap_styles', Girlss_trx_addons_add_googlemap_styles);
jQuery(document).on('action.init_shortcodes', Girlss_trx_addons_init);
jQuery(document).on('action.init_hidden_elements', Girlss_trx_addons_init);

// Add theme specific styles to the Google map
function Girlss_trx_addons_add_googlemap_styles(e) {
...
function Girlss_trx_addons_init(e, container) {
    "use strict";
    if (arguments.length < 2) var container = jQuery('body');
    if (container===undefined || container.length === undefined || container.length == 0) return;

    container.find('.sc_countdown_item canvas:not(.inited)').addClass('inited').attr('data-color', GirlsS_STORAGE['alter_link_color']);
}
...
function trx_addons_sc_skills_init(e, container) {
    ...
    var scrollPosition = Girlss_win.scrollTop() + Girlss_win.height();
...
jQuery(document).on('action.init_shortcodes', function(e, container) {
    ...
    if (TRX_ADDONS_STORAGE['scroll_to_anchor'] == 1) {
        ...
        Girlss_win.bind('mousewheel DOMMouseScroll', function(e) {
            if (screen.width < 960 || Girlss_win.width() < 960 || wheel_stop || trx_addons_browser_is_ios()) {
                return;
            }
            ...
            var doit = false;
            var scroll_offset = parseInt(Girlss_win.scrollTop(), 0);
            var wh = Girlss_win.height();
    ...
    function trx_addons_detect_active_toc() {
        ...
        toc_menu_items.each(function(idx) {
            ...
            if (off < scroll_offset + Girlss_win.height()*0.8 && scroll_offset < off_next - 50) {
