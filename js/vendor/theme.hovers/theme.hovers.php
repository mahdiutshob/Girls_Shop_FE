...
if (!function_exists('Girlss_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'Girlss_hovers_theme_setup3', 3 );
	function Girlss_hovers_theme_setup3() {
		// Add 'Buttons hover' option
		Girlss_storage_set_array_before('options', 'sidebar_widgets', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'Girlss'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'Girlss') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'Girlss'),
					'slide_left'	=> esc_html__('Slide from Left',	'Girlss'),
					'slide_right'	=> esc_html__('Slide from Right',	'Girlss'),
					'slide_top'		=> esc_html__('Slide from Top',		'Girlss'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'Girlss'),
//					'arrow'			=> esc_html__('Arrow',				'Girlss'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'Girlss'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'Girlss') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
					'section' => esc_html__('Content', 'Girlss')
				),
				"options" => array(
					'dots'	=> esc_html__('Dots',	'Girlss'),
					'icon'	=> esc_html__('Icon',	'Girlss'),
					'icons'	=> esc_html__('Icons',	'Girlss'),
					'zoom'	=> esc_html__('Zoom',	'Girlss'),
					'fade'	=> esc_html__('Fade',	'Girlss'),
					'slide'	=> esc_html__('Slide',	'Girlss'),
					'pull'	=> esc_html__('Pull',	'Girlss'),
					'border'=> esc_html__('Border',	'Girlss')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('Girlss_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'Girlss_hovers_theme_setup9', 9 );
	function Girlss_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'Girlss_hovers_frontend_scripts', 1010 );
		add_filter( 'Girlss_filter_localize_script','Girlss_hovers_localize_script' );
		add_filter( 'Girlss_filter_merge_scripts',	'Girlss_hovers_merge_scripts' );
		add_filter( 'Girlss_filter_merge_styles',	'Girlss_hovers_merge_styles' );
		add_filter( 'Girlss_filter_get_css', 		'Girlss_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'Girlss_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'Girlss_hovers_frontend_scripts', 1010 );
	function Girlss_hovers_frontend_scripts() {
		if ( Girlss_is_on(Girlss_get_theme_option('debug_mode')) && file_exists(Girlss_get_file_dir('includes/theme.hovers/theme.hovers.js')) )
			wp_enqueue_script( 'Girlss-hovers', Girlss_get_file_url('includes/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( Girlss_is_on(Girlss_get_theme_option('debug_mode')) && file_exists(Girlss_get_file_dir('includes/theme.hovers/theme.hovers.css')) )
			wp_enqueue_style( 'Girlss-hovers',  Girlss_get_file_url('includes/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('Girlss_hovers_merge_scripts')) {
	//Handler of the add_filter( 'Girlss_filter_merge_scripts', 'Girlss_hovers_merge_scripts' );
	function Girlss_hovers_merge_scripts($list) {
...
if (!function_exists('Girlss_hovers_merge_styles')) {
	//Handler of the add_filter( 'Girlss_filter_merge_styles', 'Girlss_hovers_merge_styles' );
	function Girlss_hovers_merge_styles($list) {
...
if (!function_exists('Girlss_hovers_localize_script')) {
	//Handler of the add_filter( 'Girlss_filter_localize_script','Girlss_hovers_localize_script' );
	function Girlss_hovers_localize_script($arr) {
		$arr['button_hover'] = Girlss_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('Girlss_hovers_add_icons') ) {
	function Girlss_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'Girlss') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink());; ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') esc_html_e('Details', 'Girlss');
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'Girlss_hovers_get_css' ) ) {
	//Handler of the add_filter( 'Girlss_filter_get_css', 'Girlss_hovers_get_css', 10, 4 );
	function Girlss_hovers_get_css($css, $colors, $fonts, $scheme='') {
